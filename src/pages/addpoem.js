...
const AddPoem = ({poems}) => {
      ...
      return (
        showInvalidAlert ? (
          <div className={"text-white px-6 py-4 border-0 rounded relative mb-4 bg-red-400"}>
            ...
            <button className="bg-transparent right-0 top-0 mt-4 mr-6 text-2xl font-bold leading-none absolute focus:outline-none" onClick={() => setShowInvalidAlert(false)}>
              <p>×</p>
            </button>
          </div>
        ) : null
  ...
  const formHandler = (event) => {
    ...
    if (newPoemTitle.length === 0 || newPoemAuthor.length === 0 || newPoemText.length === 0) {
      ...
      poemServices.create(poemObject)
      .then((obj) => {
        if (obj.id && obj.author && obj.text) {
          setNewPoemTitle('')
          setNewPoemAuthor('')
          setNewPoemText('')
